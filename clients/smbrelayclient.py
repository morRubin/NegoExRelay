...
from clients import ProtocolClient
...
class SMBRelayClient(ProtocolClient):
    ...
    def sendNegoNegotiate(self, negotiateMessage):
        if self.session.getDialect() == SMB_DIALECT:
            out = self.sendNegotiatev1(negotiateMessage)
        else:
            out = self.sendNegotiatev2(negotiateMessage)

        self.negotiateMessage = negotiateMessage
        self.challengeMessage = out

        # Store the Challenge in our session data dict. It will be used by the SMB Proxy
        self.sessionData['CHALLENGE_MESSAGE'] = negotiateMessage
        self.serverChallenge = out
    ...
    def sendNegoAuth(self, authenticateMessageBlob, serverChallenge=None):
        if self.session.getDialect() == SMB_DIALECT:
            token, errorCode = self.sendAuthv1(authenticateMessageBlob, serverChallenge)
        else:
            token, errorCode = self.sendNegoAuthv2(authenticateMessageBlob, serverChallenge)

        return token, errorCode

    def sendNegoAuthv2(self, authenticateMessageBlob, serverChallenge=None):
        authData = authenticateMessageBlob
